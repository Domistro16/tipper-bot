...
async function getUserWallet(userId) {
  ...
  try {
    const res = await axios.get(`http://tipper-server-production.up.railway.app/:800/api/wallets/${userId}`);
    ...
    if (err.response && err.response.status === 404) {
      ...
      try {
        console.log('trying...');
        await axios.post(`http://tipper-server-production.up.railway.app/:800/api/wallets/newWallet`, {
          userId,
...
async function getDroptip(droptipId) {
  ...
  try {
    const res = await axios.get(`http://tipper-server-production.up.railway.app/:800/api/droptips/${droptipId}`);
...
async function getKey(UserId) {
  ...
  try {
    const res = await axios.get(`http://tipper-server-production.up.railway.app/:800/api/wallets/privateKey/${UserId}`);
...
async function setDroptip(droptipId, droptip) {
  ...
  try {
    const res = await axios.post(`http://tipper-server-production.up.railway.app/:800/api/droptips/updateDroptip`, formattedDroptip);
...
async function newDroptip(droptipId, droptip) {
  ...
  try {
    const res = await axios.post(`http://tipper-server-production.up.railway.app/:800/api/droptips/newDroptip/`, formattedDroptip);
    return res; // Assuming API returns { wallet: { address: "0x..." } }
  } catch (error) {
    console.log(error);
  }
