...
async function getKey(UserId) {
  ...
  try {
    const res = await axios.get(`http://localhost:800/api/wallets/privateKey/${UserId}`);
...
async function newDroptip(droptipId, droptip) {
  ...
  try {
    const res = await axios.post(`http://localhost:800/api/droptips/newDroptip/`, formattedDroptip);
    return res; // Assuming API returns { wallet: { address: "0x..." } }
  } catch (error) {
    console.log(error);
  }
